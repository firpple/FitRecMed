...
'''
class ATPVDataSetClass(equations.BioEquClass):
	...
	def __init__(self):
		...
		self.ATP_array = np.empty(shape=(0))
	...
	def calcCurve(self):
		...
		if (self.ATP_array.size > 0) and (self.V_array.size > 0):

			#calcs mim parameters
			out = minimize(self.residual,self.param, args=(self.ATP_array, self.V_array))
		...
		self.x_fit = np.array(range(1,  int( max(self.ATP_array)),  1))
		self.y_fit = np.empty(shape =(0))
		for i in self.x_fit:
		#sorry ranout of room
			self.param['ATP'].value = i
			self.recalc()
			self.y_fit =np.append(self.y_fit,
									equations.calc_Vn(  self.r, self.param['L'],
													self.ton, 0.001,  
													self.param['N'], self.Vlittle, 
														self.param['d']))
		return
	...
	def residual(self, params, x_array, y_array):
		...
		for i in x_array:
			...
			ton = equations.calc_ton(kAd, kT, i)
			kDet = equations.calc_kDet(ton)
			r = equations.calc_r(ton, kAtt)
			Vlittle = equations.calc_Vlittle(kAtt,kAd, kT, i)
	...
	def readFrom(self, name):
		...
		reader.next()
		for row in reader:
			self.ATP_array = np.append(self.ATP_array, float(row[0]))
			self.V_array = np.append(self.V_array, float(row[1]))
			#print self.V_array
		
		ifile.close()
		
		if(np.mean(self.V_array[0]) < 50):
...
class NVMenuHandler:
	...
	def __init__(self, master):
		...
		self.NFixed = Tkinter.BooleanVar()


		self.set = ATPVDataSetClass()
		############creating menu system##############
	
		self.master = master
		self.master.wm_title("ATP vs V regression fitter")
		...
		#N
		self.l_N = Tkinter.Label(master, text = "N (uM)", justify = 'left')
		self.l_N.grid(row = 6, column = 0, sticky = Tkinter.W)

		self.e_N = Tkinter.Entry(master)
		self.e_N.grid(row = 6, column = 1)
		self.e_N.insert(0,1000)

		self.c_N = Tkinter.Checkbutton(master, variable = self.NFixed)
		self.c_N.grid(row = 6, column = 2)
		...
		self.e_fileName.insert(0, "CMM V vs ATP.csv")
	...
	def FormatSaveOutput(self):
		...
		if(self.set.ATP_array.size > arrayHeight):
			arrayHeight = self.set.ATP_array.size
		...
		#data labels
		outputMatrix[0][ATPCol] = 'ATP (uM)'
		outputMatrix[0][VCol] = 'V (um/s)'

		#Setup
		heightIndex = 1
		ATPIter = np.nditer(self.set.ATP_array)
		VIter = np.nditer(self.set.V_array)

		#data points
		while not ATPIter.finished:
			outputMatrix[heightIndex][ATPCol] = str(float(ATPIter[0]))
	...
	def dataTransferTo(self):
		...
		self.set.set_N(float(self.e_N.get()))
		...
		self.set.set_NVary(not self.NFixed.get())
		return
	...
	def makeGraph(self):
		...
		##plot original data
		ax = plt.figure()
		plt.scatter(self.set.ATP_array, 
		...
		plt.xlim(min(self.set.ATP_array) - 10, max(self.set.ATP_array) + 5)
